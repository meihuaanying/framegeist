...
function applyTheme() {
  ...
  const btn = $("themeBtn");
  btn.textContent = mode === "auto" ? "â—? : mode === "light" ? "â˜€" : "â˜?;
...
function buildTemplatePicker() {
  ...
  for (const tpl of list) {
    ...
    cell.title = `${t(`cat.${tpl.category}`) !== `cat.${tpl.category}` ? t(`cat.${tpl.category}`) : tpl.category} Â· ${tplName(tpl)}`;
...
function updatePinned() {
  ...
  $("pinnedName").textContent = tpl ? tplName(tpl) : "â€?;
...
function buildWall() {
  ...
  for (const tpl of list) {
    ...
    cell.innerHTML = (src
      ...
      `<div class="wall-actions"><button class="use">${t("wall.use")}</button><button class="icon" title="${t("wall.preview")}">â¤?/button></div>`;
...
/* Fuji recipe keys exposed by the engine `exif.get()` (v0.5.0 M2). Rows are
   only emitted for values that are actually present â€?never fabricated. */
const FUJI_KEYS = [
...
function showExif() {
  ...
  for (const [key, val] of rows) {
    ...
    if (val === null || val === undefined || val === "") { dd.textContent = "â€?; dd.className = "none"; }
  ...
  for (const layer of textLayers) {
    ...
    items.forEach((item, idx) => {
      ...
      if (text === null) {
        ...
        ro.className = "ro";
        ro.textContent = item.expr + "  Â·  " + t("exifEdit.advanced");
        ro.title = item.expr;
        row.appendChild(ro);
      } else {
      ...
      mk("â†?, () => moveLine(edits, layer.id, items, idx, -1));
      mk("â†?, () => moveLine(edits, layer.id, items, idx, +1));
      mk("âœ?, () => {
    ...
    add.onclick = () => {
      const newItems = [...items, { expr: "'æ–°æ–‡å­?", fallback: null }];
      setLayerEdits(state.templateId, { ...edits, [layer.id]: newItems });
      buildLineEditor(); renderNow();
    };
    div.appendChild(add);
...
/* ------------------------------------------------------ font warmup (v0.6.0) */
/* Contract Q10 ã€Œå¼•æ“Žå­—ä½“ç¦»çº¿é¢„ç¼“å­˜ã€? after boot (first paint done) fetch the
...
/// on demand â€?the WASM engine has no filesystem (v0.3.0 T-A fix).
...
async function renderNow() {
  ...
  try {
    ...
    withViewTransition(() => setStage(url, `${t("stage.rendered")} Â· ${ms} ms Â· ${(out.length / 1024).toFixed(0)} KB`));
...
async function checkUpdates() {
  ...
  try {
    ...
    $("updateBox").innerHTML = `${t("update.latest", { v: escapeHtml(rel.tag_name) })} Â· <a href="${escapeHtml(rel.html_url)}" target="_blank" rel="noreferrer">${t("update.view")}</a>`;
  } catch {
