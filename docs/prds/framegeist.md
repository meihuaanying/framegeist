...
- **2026-09-12 ｜ v0.3.0 冲刺（frameelf 差距二轮 + 游戏主题 + 禁紫 + 设置/灯箱） ｜** 用户提出 6 项：禁紫、预览图分类化、frameelf 对标扩容+游戏系列、文字对比度、Logo 不显示、设置页。三轮 grill 后按 AGENTS v0.3.0 执行：
  - **Logo 不显示的三层根因（自证 + 修复）**：① 引擎 `@builtin/` 资产解析把路径拼成 `brand/brand/…`（连 CLI 都空白，实测 CLI 与无资产渲染像素差=0）；② WASM 端无文件系统且 UI 未注册品牌 PNG；③ 徽章只覆盖 37/63 套。修复：路径归一 + UI 按需 `fetch(./brand|series|game/*.png)`→`register_asset` + 徽章扩展到 52 套 + **E2E 增加"开关 Logo 必须改变像素"断言**（修补 0.2.0 只断言识别文字的测试漏洞）。
  - **禁紫配色**：全局替换为知乎实采蓝系 `#1772F6 → #50C8FD`（底 #F8F8FA、灰 #373A40、辅橙 #FB6622），应用+官网+favicon；E2E 新增禁用色文件扫描 + 运行时 token 断言。
  - **自动对比度**：引擎按 WCAG 相对亮度采样文字区域背景，对比 <2.5:1 自动切黑/白（`color:"auto"` 强制自动；手动覆盖不被改写）；品牌/系列/游戏徽章按背景自动选深浅变体（`tint:auto`）。黄金基线 12 条因叠加文字自动调色重生成（已注明）。
  - **模板库 63→130**：frameelf 对标 37（相机相框 8/手持 5/镜头徽章 4/胶片加强 6/无边框 6/签名 4/机身轮廓 4）+ 游戏 18（原神/绝区零/崩坏/明日方舟/燕云十六声/黑神话悟空 ×3）+ 开源灵感 12（磨砂编辑/画廊夜黑/展览三式/卡片三式/Instax/城市字标/邮戳）。新增原创素材：系列徽章 7 + 游戏字标 6（resvg 渲染，OFL 字体）+ 相框线稿 3（机身份轮廓/手机/胶片条）+ 字体 Great Vibes / Ma Shan Zheng（GB2312 子集 3.15MB）。游戏模板仅含原创字标与"非官方"声明，零官方素材。
  - **设置页 + Lightbox**：顶栏齿轮 + `Ctrl+,` 面板（外观语言/导出默认/文字与 Logo 默认/隐私 GPS/本地数据逐项清空+占用统计/关于许可/桌面保存方式）；模板缩略图"应用/预览"双通道 + 预览大图（640px，←/→/ESC）+ 免责声明展示 + 双语模板名与分类名。
  - **演示照片**：6 张（3 风光 + 3 建筑）= Cleveland CC0 4 张 + Picsum 2 张，按分类映射到缩略图/预览/样张三档；CREDITS 更新。
  - **验收**：E2E 扩至 **39/39 全绿**（含无紫扫描、徽章像素、自动对比度渲染、灯箱交互、设置持久化、130 配额、游戏分类/双语名/声明）；性能维持（启动 ~0.5s、渲染毫秒级）；cargo test/clippy/四 target/wasm 冒烟全绿。截图归档 `docs/reports/v0.3.0/`。
...
- **2026-09-11 ｜ v0.2.0 冲刺（frameelf 差距审计 + 三轮 grill 后执行） ｜** 用户以 frameelf 为基准指出：布局留白过大需滚动、模板缩略图全部一样无法辨认、缺少品牌 Logo、缺少"愉悦感"（微动画/舒适度）、批量与选择器简陋。按 `AGENTS.md` v0.2.0 契约一气呵成：
  - **审计实测**（1440×900 真实 WebView2）：成品图底边 y=1201（超视口 301px）、画布区覆盖率仅 43.7%、侧栏 1524px 操作按钮在首屏外、页面总高 1639px 必须滚动；同分类相邻模板缩略图像素差异仅 0.4–0.7%（肉眼不可分）；并发现**分类筛选后已选模板不可见的"选 classic 渲染出 shell"状态坑**、引擎错误英文混入中文 UI、HEIC 可选但不可解、UI 覆盖参数 camelCase 被引擎拒绝（隐藏 400）等问题。
  - **修复与升级**：① 固定视口双栏编辑器（画布 contain 自适配 + 滚轮缩放/拖拽平移/双击复位/水平垂直翻转 + 操作栏吸底），实测覆盖率 87.1%、零页面滚动；② 4 张 NASA 公有领域演示照片（横/竖/方/夜轮换）重渲染全部样张与缩略图，缩略图网格带悬停/懒加载渐显/双档模式（紧凑/大图）+ 分类筛选 + 搜索 + **当前选中模板常驻显示**；③ **18 个品牌资产**（10 个 Simple Icons CC0 图标 + 8 个 OFL 字体自绘字标）+ 引擎图片层（`attachTo` 水印行前置图标、缺匹配留白不造假）+ EXIF 品牌/镜头识别映射 + 全局开关/手动选择/自定义上传；④ **9 款 OFL 引擎字体**（含中文思源黑/宋 GB2312 子集 3.3/4.9MB）+ 字体选择器 + 本地上传（IndexedDB）；⑤ 画布面板（比例 6 档/背景 模糊·纯色·图片·无/取色器/翻转）；⑥ **EXIF 文字行编辑器**（字段芯片插入/排序/增删/实时重渲）+ **保存为模板**（store-only zip .fgt，浏览器下载 + 桌面系统保存对话框）+ 导入；⑦ **批量导出**（统一面板应用全部、进度条、`原名-模板名.jpg` 去重）；⑧ **导出预设**（原图/6K/4K/2K/自定义长边，不裁切）；⑨ **微动画全套**（View Transitions 切换/主题过渡、骨架屏、缩略图入场、toast/进度/按钮微交互，尊重 `prefers-reduced-motion`）；⑩ 引擎错误码 JSON + 双语本地化；HEIC 明确文案。
  - **E2E 门禁固化**：`tools/e2e-audit.mjs`（CDP 真机 23 项断言：启动 ≤2s、零滚动、覆盖率 ≥85%、缩略图加载、选中可见、预览渲染、比例/背景/翻转/缩放、品牌识别、EXIF 编辑、双档、双语、HEIC 错误路径、拼图、批量、.fgt 导入），发布前必跑，截图与报告归档 `docs/reports/v0.2.0/`。最终 **23/23 通过**（引擎就绪 ~0.5s，24MP 预览 186–582ms，拼图 0.9–1.7s）。
  - 过程中被 E2E 反向抓出的问题：CDP 注入相对路径触发 `NotReadableError`（测试脚本缺陷）+ 新版引擎对空 EXIF 照片品牌留白（符合决策）——均修正测试口径而非放宽标准。
- **2026-09-12 ｜ v0.4.0 冲刺（design-first：frameelf 深研 → 全部模板重做 + 模板墙 → 编辑器） ｜** 用户指令"深研 frameelf 后全部模板重做、子 Agent 分工设计"。执行：
  - **研究**：frameelf 桌面版为 Pake 套壳 → Web `frameelf.lazykits.com/design`；API 拉取 22 分类 217 套 + 逐套预览研读，蒸馏为 `docs/DESIGN-LANGUAGE.md`（布局原型 A–P、字阶、色彩、EXIF 映射、7 条 QA）。研究语料仅存临时目录，**严禁入仓**（红线 6）。
  - **引擎 A1–A9**（TEMPLATE-SPEC/schema 同步 v0.4）：字距 `letterSpacing`（em）、文本/形层 `rotation`、文本 `opacity`、`shape` 层（line/rect/ellipse/diamond/hexagon + 3×3 超采样）、`palette` 层（median-cut 主色 + chips/hex 标签）、`canvas.radius`/`shadow` 真正渲染（圆角用 composite_over 混合，修复角像素透明遮红）、`background.type:"tint"`（照片主色铺底）、GPS 表达式（`gps_lat/lon/alt`/DMS，受 `keep_gps` 门控）+ `weekday(_cn)` + date token（`WW/MMM/MMMM/Do/D/M`）；分类枚举扩至 25（旧值保留兼容）。图层改为**声明顺序绘制**，`attachTo` 徽章延迟最后。自绘文本渲染器替代 `draw_text_mut`（逐字 outline_glyph + 旋转 pad 到对角线）。坑：date 格式串含 ", " 崩溃；Zeller 星期差一（h=0 为星期六）；`ab_glyph` 无 `font.glyph()`。
  - **模板重做**：7 个子 Agent 波次产出 163 套 + 3 套黄金范例；旧 239 套（221 非游戏 + 18 游戏）清理为 **184 套 = 166 新 + 18 游戏**；3 套手写种子迁 `crates/framegeist-cli/tests/fixtures/`（黄金基线内容不变、无需重生成）；退役 `gen-extras.mjs` 与生成式 `gen-templates.mjs`（改为扫描式 manifest+镜像）。24 张分类接触表逐张视觉审计通过。
