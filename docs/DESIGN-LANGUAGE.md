...
### 3.6 徽标规范（Q2/Q5，v0.9.0 彩色化）

- 尺寸（v0.9 上调）：照片高 ≥6% 且 ≥18px（引擎强制）；默认层高 **6.5%**；长名最大宽度 ≤**44%** 照片宽（超宽缩放/截断由引擎处理）。
- **颜色（v0.9.0，v0.9.2 恢复 Canon 官方红）**：官方仍在维护的 Simple Icons 品牌（19 个）按阈值规则（HSL `s ≥ 0.18` 且 `0.10 ≤ l ≤ 0.90`）判为彩色，主变体直接用官方 hex；黑/白官方色品牌保持单色。Simple Icons 已下架的品牌使用原创排版字标；其中 Canon 按公开官方红（Pantone 186 C ≈ `#C8102E`）着色（`originalColorOverride`），字形仍为原创。同一品牌的官方字标与原创 lockup 同色；series/game 徽章保持单色。
- 对比：官方彩色在局部背景对比 ≥3:1（WCAG 1.4.11）时保留；不足自动回退黑/白变体；纯色底黑白变体必达 4.5:1；花底自动加反向 1px 描边或半透明底板；`tint:"color"` 可强制官方彩色。
- 数量：同模板内 1 个相机徽标 + 可选 1 个系列徽标；与文本最小间距 0.8×徽标高；透明度 0.7–1.0（编辑器可调）。
- 位置：跟随模板锚点，编辑器可切四角固定（图层级 `corner`，支持 `"anchor"` 哨兵覆盖模板级位置）；提供「官方字标（Simple Icons CC0）/ 原创排版 lockup（OFL 字型）」两种风格。
